...
if (requerimentoAlteracaoForm) {
  ...
  requerimentoAlteracaoForm.addEventListener('submit', (event) => {
    ...
    preview.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Requerimento para alteração de característica veicular</title><style>
      ...
      <p class="section">Alteração Pretendida:</p><div class="request-box">${valueOf(data, 'alteracao')}</div><p class="section">Requerimento:</p><p class="body-copy">O(A) abaixo assinado(a), requer o desbloqueio do laudo de vistoria, tendo em vista a documentação em anexo, por esta estar de acordo com as exigências contidas na Portaria nº 91/2004.</p><p class="body-copy">Declaro ainda, ser autêntica e verdadeira toda a documentação apresentada. Responsabilizo-me, sob as penas da Lei Civil e Criminal, pela presente requisição.</p><p class="body-copy">Nestes termos, pede deferimento.</p><section class="signature-footer"><p class="date">${valueOf(data, 'cidadeAssinatura')}, ${formattedDate(data.get('dataAssinatura'))}.</p><div class="signature"><strong>${nameOf(data, 'proprietario')}</strong><br />Assinatura do(a) proprietário(a) ou representante legal</div>${window.renderMegaDeclaration(valueOf(data, 'cidadeAssinatura'), formattedDate(data.get('dataAssinatura')))}</section>
      </article></body></html>`);
