...
if (procuracaoParticularForm) {
  ...
  procuracaoParticularForm.addEventListener('submit', (event) => {
    ...
    preview.document.write(`<!doctype html>
      ...
      <style>
        @page { size: A4 portrait; margin: 0; }* { box-sizing: border-box; }body { margin: 0; background: #eee; color: #111; font-family: Arial, sans-serif; }.document { position: relative; width: 210mm; min-height: 297mm; margin: 10px auto; padding: 6mm 14mm 5mm; background: #fff; font-size: 8pt; line-height: 1.18; }.header { position: relative; min-height: 17mm; border-bottom: 2px solid #111; }.title { margin: 0 24mm; padding-top: 4mm; text-align: center; font-size: 15pt; }.service-note { position: absolute; top: 0; right: 1mm; width: 20mm; text-align: right; font-size: 7.5pt; }.section-heading { margin: 5px 0 0; font-size: 9.8pt; font-weight: 700; }.grid { display: grid; border: 1px solid #222; border-bottom: 0; }.grid div { min-height: 24px; padding: 3px 4px; border-right: 1px solid #222; border-bottom: 1px solid #222; }.grid div:last-child { border-right: 0; }.grid b { display: block; margin-bottom: 1px; font-size: 7pt; }.full { grid-template-columns: 1fr; }.person-line { grid-template-columns: 1.2fr .9fr .9fr; }.person-address { grid-template-columns: .7fr .9fr .38fr .7fr .88fr; }.contact-line { grid-template-columns: 1.25fr 1fr; }.vehicle-line { grid-template-columns: .55fr 1.5fr .7fr; }.powers { min-height: 72px; padding: 6px; border: 1px solid #222; text-align: justify; font-size: 8.25pt; line-height: 1.3; }.place-date { margin: 1px 0 47px; }.signature { width: 98mm; margin: 0 auto 3px; border-top: 1px solid #222; padding-top: 3px; text-align: center; }.mega-stamp { display: block; width: 66mm; height: auto; margin: 3px auto 4px; }.docs { border-top: 2px solid #111; padding-top: 3px; font-size: 6.35pt; line-height: 1.15; }.docs strong { font-size: 7pt; }.docs ol { margin: 2px 0; padding-left: 15px; }.footer { border-top: 1px solid #111; padding-top: 3px; text-align: center; font-size: 6pt; }.print-hint { position: fixed; z-index: 10; top: 18px; right: 18px; border: 0; background: #b42313; color: #fff; padding: 12px 16px; font-weight: 700; cursor: pointer; }@media print { body { background: #fff; }.document { width: 210mm; min-height: 297mm; margin: 0; }.print-hint { display: none; } }
      </style></head><body><button class="print-hint" onclick="window.print()">Imprimir / Salvar PDF</button><article class="document">
        <header class="header"><h1 class="title">PROCURAÇÃO PARTICULAR</h1><div class="service-note">(Serviços relacionados a VEÍCULOS)</div></header>
        <h2 class="section-heading">OUTORGANTE:</h2><section class="grid full"><div><b>Nome:</b><strong>${nameOf(data, 'outorganteNome')}</strong></div></section><section class="grid person-line"><div><b>CPF / CNPJ:</b>${valueOf(data, 'outorganteCpfCnpj')}</div><div><b>RG:</b>${valueOf(data, 'outorganteRg')}</div><div><b>Órgão Expedidor:</b>${valueOf(data, 'outorganteOrgao')}</div></section><section class="grid full"><div><b>Rua, Avenida, etc:</b>${valueOf(data, 'outorganteEndereco')}</div></section><section class="grid person-address"><div><b>Bairro/Setor:</b>${valueOf(data, 'outorganteBairro')}</div><div><b>Município:</b>${valueOf(data, 'outorganteCidade')}</div><div><b>UF:</b>${valueOf(data, 'outorganteEstado')}</div><div><b>CEP:</b>${valueOf(data, 'outorganteCep')}</div><div><b>Telefone:</b>${valueOf(data, 'outorganteTelefone')}</div></section><section class="grid contact-line"><div><b>E-mail:</b>${valueOf(data, 'outorganteEmail')}</div><div><b>Telefones:</b>${valueOf(data, 'outorganteTelefone')}</div></section>
        <h2 class="section-heading">OUTORGADO (PROCURADOR):</h2><section class="grid full"><div><b>Nome:</b><strong>${nameOf(data, 'procuradorNome')}</strong></div></section><section class="grid person-line"><div><b>CPF / CNPJ:</b>${valueOf(data, 'procuradorCpfCnpj')}</div><div><b>RG:</b>${valueOf(data, 'procuradorRg')}</div><div><b>Órgão Expedidor:</b>${valueOf(data, 'procuradorOrgao')}</div></section><section class="grid full"><div><b>Rua, Avenida, etc:</b>${valueOf(data, 'procuradorEndereco')}</div></section><section class="grid person-address"><div><b>Bairro/Setor:</b>${valueOf(data, 'procuradorBairro')}</div><div><b>Município:</b>${valueOf(data, 'procuradorCidade')}</div><div><b>UF:</b>${valueOf(data, 'procuradorEstado')}</div><div><b>CEP:</b>${valueOf(data, 'procuradorCep')}</div><div><b>Telefone:</b>${valueOf(data, 'procuradorTelefone')}</div></section><section class="grid contact-line"><div><b>E-mail:</b>${valueOf(data, 'procuradorEmail')}</div><div><b>Telefones:</b>${valueOf(data, 'procuradorTelefone')}</div></section>
        ...
        <p class="place-date">${valueOf(data, 'cidadeAssinatura')}, ${formattedDate(data.get('dataAssinatura'))}.</p><div class="signature">Assinatura do(a) Outorgante</div><img class="mega-stamp" src="${stampUrl}" alt="Carimbo assinado da MEGA Despachante">
