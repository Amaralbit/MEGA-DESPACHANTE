...
if (procuracaoParticularForm) {
  ...
  // E-mail mantém a caixa digitada pelo usuário; deixá-lo maiúsculo só ficaria estranho.
  const emailOf = (data, key) => escapeHtml(data.get(key));
  ...
  const buildDocumentHtml = (data) => `<!doctype html>
      ...
      <style>
        ...
        <h2 class="section-heading">OUTORGANTE:</h2><section class="grid full"><div><b>Nome:</b><strong>${nameOf(data, 'outorganteNome')}</strong></div></section><section class="grid person-line"><div><b>CPF / CNPJ:</b>${valueOf(data, 'outorganteCpfCnpj')}</div><div><b>RG:</b>${valueOf(data, 'outorganteRg')}</div><div><b>Órgão Expedidor:</b>${valueOf(data, 'outorganteOrgao')}</div></section><section class="grid full"><div><b>Rua, Avenida, etc:</b>${valueOf(data, 'outorganteEndereco')}</div></section><section class="grid person-address"><div><b>Bairro/Setor:</b>${valueOf(data, 'outorganteBairro')}</div><div><b>Município:</b>${valueOf(data, 'outorganteCidade')}</div><div><b>UF:</b>${valueOf(data, 'outorganteEstado')}</div><div><b>CEP:</b>${valueOf(data, 'outorganteCep')}</div><div><b>Telefone:</b>${valueOf(data, 'outorganteTelefone')}</div></section><section class="grid contact-line"><div><b>E-mail:</b>${emailOf(data, 'outorganteEmail')}</div><div><b>Telefones:</b>${valueOf(data, 'outorganteTelefone')}</div></section>
        <h2 class="section-heading">OUTORGADO (PROCURADOR):</h2><section class="grid full"><div><b>Nome:</b><strong>${nameOf(data, 'procuradorNome')}</strong></div></section><section class="grid person-line"><div><b>CPF / CNPJ:</b>${valueOf(data, 'procuradorCpfCnpj')}</div><div><b>RG:</b>${valueOf(data, 'procuradorRg')}</div><div><b>Órgão Expedidor:</b>${valueOf(data, 'procuradorOrgao')}</div></section><section class="grid full"><div><b>Rua, Avenida, etc:</b>${valueOf(data, 'procuradorEndereco')}</div></section><section class="grid person-address"><div><b>Bairro/Setor:</b>${valueOf(data, 'procuradorBairro')}</div><div><b>Município:</b>${valueOf(data, 'procuradorCidade')}</div><div><b>UF:</b>${valueOf(data, 'procuradorEstado')}</div><div><b>CEP:</b>${valueOf(data, 'procuradorCep')}</div><div><b>Telefone:</b>${valueOf(data, 'procuradorTelefone')}</div></section><section class="grid contact-line"><div><b>E-mail:</b>${emailOf(data, 'procuradorEmail')}</div><div><b>Telefones:</b>${valueOf(data, 'procuradorTelefone')}</div></section>
